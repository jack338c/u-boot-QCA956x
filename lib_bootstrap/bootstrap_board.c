...
#ifdef DEBUG_ENABLE_BOOTSTRAP_PRINTF
	printf ("\n relocating to address %08x, uboot_end_data_bootstrap:%08x.\r\n", addr, lzmaImageaddr);
#endif

#ifdef CFG_DOUBLE_BOOT_SECOND
    bootstrap_board_init_r (id, addr);
#else
...
void bootstrap_board_init_r (gd_t *id, ulong dest_addr)
{
	...
	ulong	addr;
	ulong	data, len, checksum, ck_real;
	ulong  *len_ptr;
    ...
    printf("\r\nmonitor base:%08x, end_data:%08x,dest_addr:%08x.\r\n", 
                                                   BOOTSTRAP_CFG_MONITOR_BASE,
                                                   (ulong)&uboot_end_data_bootstrap,
                                                   dest_addr);
#endif

#ifdef CFG_DOUBLE_BOOT_SECOND
    addr = (char *)((ulong)&uboot_end_data_bootstrap - 
                BOOTSTRAP_CFG_MONITOR_BASE + CFG_BOOT_SECOND_BASE);
#else
    addr = (char *)(BOOTSTRAP_CFG_MONITOR_BASE + 
               ((ulong)&uboot_end_data_bootstrap - dest_addr));
#endif
    ...
    ck_real = crc32 (0, (char *)data, len);
    
	if (ck_real != checksum) {
#ifdef DEBUG_ENABLE_BOOTSTRAP_PRINTF
		printf ("Bad Header Checksum,%08x != %08x.\n", checksum, ck_real);
#endif
		return;
	}
